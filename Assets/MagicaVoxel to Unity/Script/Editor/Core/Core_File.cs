namespace VoxeltoUnity {
	...
	public static class Core_File {
						...
						if (skinMR) {
							...
							if (voxelModel.RootBones != null) {
								for (int i = 0; i < voxelModel.RootBones.Length; i++) {
									...
									if (boneTF) { boneTF.localPosition -= rootBoneOffset; }
								}
							}
		...
		private static Transform CreateModelFrom (
			VoxelCore.Result.VoxelNode vTransform, Transform parent,
			...
			root.localPosition = vTransform.Position.ToVector3() * ModelScale;
			root.localRotation = vTransform.Rotation;
			root.localScale = vTransform.Scale.ToVector3();
			root.gameObject.SetActive(vTransform.Active);

			if (vTransform.Model != null) {

				root.name = !string.IsNullOrEmpty(vTransform.Name) ? vTransform.Name : GetIndexedName("Model", parent.childCount - 1);

				// Empty Check
				bool isEmpty = true;
				var uMesh = vTransform.Model;
				if (uMesh.Count > 0) {
					for (int i = 0; i < uMesh.Count; i++) {
				...
				if (!isEmpty) {
					...
					// Add Assets
					Texture2D texture = vTransform.Texture;
					if (!texture) {
					...
					// Add Mesh To
					if (vTransform.Model.Count == 1) {
						var mesh = vTransform.Model[0];
						if (!meshs.Contains(mesh)) {
							meshs.Add(mesh);
						}
						AddMeshTo(mesh, root, mat, isRig);
					} else {
						for (int i = 0; i < vTransform.Model.Count; i++) {
			...
			} else if (vTransform.Children != null && vTransform.Children.Length > 0) {
				// Sub Objects
				root.name = !string.IsNullOrEmpty(vTransform.Name) ? vTransform.Name : "Container";
				for (int i = 0; i < vTransform.Children.Length; i++) {
					CreateModelFrom(vTransform.Children[i], root, ref meshs, ref materialsMap, isRig, withAvatar);
				}
			}
			return root;
		}
		...
		private static Transform CreateBoneTransform (VoxelCore.Bone rootBone, Transform parent, ref List<Transform> boneTFList) {
