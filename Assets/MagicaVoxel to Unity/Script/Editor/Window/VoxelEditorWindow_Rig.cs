			...
			if (!string.IsNullOrEmpty(path)) {
				...
				if (!string.IsNullOrEmpty(path)) {
					try {
						...
						var lightmap = (VoxelCore.LightMapSupportType)EditorPrefs.GetInt("V2U.LightMapSupportTypeIndex", 0);
						...
						VoxelCore.SetMergeInDirection(mergeF, mergeB, mergeU, mergeD, mergeL, mergeR);

						var result = VoxelCore.CreateModel(Data, scale, lightmap, true);
						...
						if (result.VoxelModels.Length > 1) {
							result.VoxelModels = new VoxelCore.Result.VoxelModel[1] { vModel };
						}
						...
						vModel.RootNode = new VoxelCore.Result.VoxelNode() {
							Model = uMesh,
							Texture = texture,
							Active = true,
							Children = null,
							Name = "",
							Position = new VoxelCore.Int3(0, 0, 0),
							Rotation = Quaternion.identity,
							Scale = new VoxelCore.Int3(1, 1, 1),
						};
						...
						Core_File.CreateFileForResult(new List<VoxelCore.Result>() { result }, shader, scale);
