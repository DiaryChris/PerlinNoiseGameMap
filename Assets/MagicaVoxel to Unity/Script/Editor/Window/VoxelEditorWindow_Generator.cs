	...
	public partial class VoxelEditorWindow {
		...
		private void CharacterGeneratorGUI () {
			LayoutV(() => {
				...
				changed = CharacterObjectGUI(CGConfig.Head) || changed;
				changed = CharacterObjectGUI(CGConfig.Neck) || changed;
		...
		private bool CharacterObjectGUI (Core_CharacterGeneration.CharacterObject obj) {
			const int HEIGHT = 16;
			const int LABEL_WIDTH = 82;
			//const int GAP_WIDTH = 22;
			const int GAP_HEIGHT = 6;
			...
			LayoutV(() => {
				...
				// Name
				changed = StringField(LABEL_WIDTH, HEIGHT, "Name", ref obj.Name) || changed;
				Space(GAP_HEIGHT);

				// Size 
				LayoutH(() => {
					changed = IntField(LABEL_WIDTH, HEIGHT, "Size", ref obj.SizeX) || changed;
					Space(2);
					changed = IntField(16, HEIGHT, "×", ref obj.SizeY) || changed;
					Space(2);
					changed = IntField(16, HEIGHT, "×", ref obj.SizeZ) || changed;
				});
				Space(GAP_HEIGHT);
		...
		private string GetRootPath () {
			...
			if (script) {
				...
				string rootName = Util.ROOT_NAME;
				if (!string.IsNullOrEmpty(path)) {
