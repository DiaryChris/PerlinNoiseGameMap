namespace VoxeltoUnity {
	...
	public partial class VoxelEditorWindow {
		...
		private bool SpawnVoxelFaces () {
			...
			switch (CurrentEditorMode) {
				...
				case EditorMode.Generator:
					// Mesh
					var result = VoxelCore.CreateModel(Data, 1f, VoxelCore.LightMapSupportType.SmallTextureButNoLightmap, false);
					var vModel = result.VoxelModels != null && result.VoxelModels.Length > 0 ? result.VoxelModels[0] : null;
					var vMesh = vModel != null && vModel.Meshs != null && vModel.Meshs.Length > CurrentModelIndex ? vModel.Meshs[CurrentModelIndex] : null;
					if (vMesh == null) { break; }
		...
		private void AddQuad (int x, int y, int z, int colorIndex, int sizeX, int sizeY, int sizeZ) {
			...
			for (int i = 0; i < 6; i++) {
				...
				if (mr) {
					...
					mat.SetColor(COLOR_SHADER_ID, GetQuadColor(color, dir));
					mr.material = mat;
				}
			}
		...
		private Color GetQuadColor (Color color, Direction dir) {
			return Color.Lerp(color,
				dir == Direction.Up ? new Color(0.1f, 0.1f, 0.1f) :
				dir == Direction.Down ? Color.black :
				dir == Direction.Left ? new Color(0.3f, 0.3f, 0.3f) :
				dir == Direction.Right ? new Color(0.2f, 0.2f, 0.2f) :
				dir == Direction.Back ? new Color(0.5f, 0.5f, 0.5f) :
				color,
			0.12f);
		}
